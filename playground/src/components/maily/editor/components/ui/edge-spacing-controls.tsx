...
export function EdgeSpacingControl(props: EdgeSpacingControlProps) {
  ...
  return (
    <Popover>
      <PopoverTrigger className="rounded hover:bg-muted">
        <ChevronUp size={14} />
      </PopoverTrigger>
      <PopoverContent
        className="flex max-w-max gap-0.5 rounded-md border border-border p-0.5! shadow-none"
        side="top"
        sideOffset={8}
      >
        <InputWithLabel
  ...
  return (
    <div className={cn("flex flex-col items-center gap-1", className)}>
      <input
        ...
        className={cn(
          "size-5 [appearance:textfield] border-0 border-none bg-muted p-0.5 text-center text-xs tabular-nums outline-hidden focus-visible:outline-none [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none",
          inputClassName
        )}
      />
      <label
